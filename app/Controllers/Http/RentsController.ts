...
export default class RentsController {
    ...
    public async getBargain({ auth, response, params }) {
        try {
            ...
            const bargainStatusColumns = ['bargain_status.id as ori_bargain_status_id', 'bargain_status.name as ori_bargain_name', 'bargain_status.*'];
            ...
            if (params.user_type === "host") {
                ...
                const bargainDataByHost = await Database
                    .from('bargains')
                ...
                const bargainDataByGuest = await Database
                    .from('bargains')
                    .select(...bargainsColumns, ...carsColumns, ...bargainStatusColumns)
                    .innerJoin('cars', 'bargains.car_id', 'cars.id')
                    .innerJoin('bargain_status', 'bargains.bargain_status_id', 'bargain_status.id')
    ...
    public async bargain({ auth, response, request }) {
        try {
            ...
            if (!carData) {
                ...
                const numberOfDays: number | null = getNumberOfDaysBetweenDates(payload.rent_from_date, payload.rent_to_date)
    ...
    public async hostBargainAcceptReject({ auth, response, request }) {
        try {
            ...
            const findBargain = await Database.from('bargains')
                .innerJoin('cars', 'bargains.car_id', 'cars.id').where("bargains.id", "=", payload.bargain_id)
                .orderBy('bargains.id', 'asc')
            ...
            if (findBargain.length !== 0) {
                ...
                if (payload.action_type === "accept") {
                    if (bargainData.last_bargain_user === tokenUserData.id) {
                        ...
                        const updatedBargain = await Bargain.query().where('id', payload.bargain_id).update(toUpdateDataAccept)
        ...
        try {
            ...
            if (findBargain.length !== 0) {
                ...
                if (bargainData.bargain_status_id === 0 || bargainData.bargain_status_id === 1) {
                    ...
                    if (bargainData.last_bargain_user === tokenUserData.id && bargainData.bargain_status_id === 0) {
                        bargainingData = {
                            "bargain_status_id": 0,
